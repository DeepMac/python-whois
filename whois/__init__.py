...
def query(domain: str, force: bool = False, cache_file: Optional[str] = None, slow_down: int = 0, ignore_returncode: bool = False) -> Optional[Domain]:
	...
	# Ignore www host entries
	if d[0] == 'www':
		d = d[1:]
	if len(d) == 1:
		return None

	# Treat IP addresses differently then domain names
	try:
		if ipaddress.IPv4Address(domain):
			# It's an IPv4 address
			tld = 'NIC_arin' # Need to add support for all NICs
	except:
		# Assuming it's a domain.
		# Map the TLD to appropriate regex
